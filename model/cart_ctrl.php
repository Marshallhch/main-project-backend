<?php
  ...
  if($req_cart == "add_cart"){
    ...
    if(isset($_SESSION['cart'])){
      ...
      if(in_array($_POST['cart_name'], $addedItem)){ // 첫번째 파라미터 배열에 두번째 파라미터값이 있으면 true 없으면 fasle
  ...
  if($req_cart == "del_cart"){
    ...
    foreach($_SESSION['cart'] as $key => $value){
      if($value['cart_idx'] == $cart_idx){
        ...
        // array_value 문 : https://www.php.net/manual/en/function.array-values.php //삭제된 세션의 인덱스를 재배치
