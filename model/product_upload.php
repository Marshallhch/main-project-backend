<?php
  ...
  // $pro_desc = addslashes($pro_desc); // 따옴표 중복 작성 시 '/'처리를 자동으로 실행
  ...
  if(!in_array($exp, $allowed_exp)){// in_array : 두번째 파라미터 배열에 첫번째 파라미터 문자열이 있으면 true
    echo json_encode(array("msg" => "허용되지 않는 이미지 파일 형식 입니다."));
    exit();
  }
  ...
  $image_dir = $_SERVER['DOCUMENT_ROOT'].'/main_poject/images/products/'; // 사진이 저장될 경로 저장
