<?php
  ...
  function get_user($conn){
    // SELECT *(전체조회) FROM [table name] WHERE [condition] ORDER BY [column name] DESC
    // table name의 전체 데이터를 condition 조건에 따라 조회하며, column name 기준 역순으로 나열한다.
    ...
    if(!mysqli_num_rows($result)){
      ...
      while($row = mysqli_fetch_array($result)){
        array_push($json_result, array("user_idx" => $row['user_idx'], "user_name" => $row['user_name'], "user_id" => $row['user_id'], "user_email" => $row['user_email'], "user_lvl" => $row['user_lvl'])); // 첫번째 파라미터 : 대상 배열, 두번째 파라미터는 배열 입력값
      }
    }
  ...
  function del_user($conn){
    ...
    if(!$user_id || $user_lvl != 1){
      echo "
        <script>
          alert('잘못된 접근입니다.');
          location.href='/main_poject/index.html';
        </script>
      ";
      exit();
    }
