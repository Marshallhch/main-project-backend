<?php
  ...
  if($_SERVER['REQUEST_METHOD'] == 'POST' && $_GET['req_sign'] == 'post_cmt'){
    post_cmt($conn);
  }
  ...
  function get_cmt($conn){
    ...
    // spl_cmt 테이블 전체 데이터와 spl_user 테이블의 아이디를 조회한다.(두 개 테이블 데이터를 동시 조회하기 위해서는 테이블간 join이 필요하다.)
  ...
  function patch_cmt($conn){
    ...
    // php에서는 공식적으로 post와 get만 지원한다. 따라서 patch, delete, put 등은 별도의 접근 처리를 해 줘야 한다.
