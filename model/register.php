<?php
  ...
  function signin($conn){
    ...
    if(!mysqli_num_rows($result)){
      ...
      $pwd_valid = password_verify($pwd, $login_data['user_pass']); // 입력된 데이터(첫번째 파라미터)와 디비의 해시 데이터(두번째 파라미터)를 비교하여 boolean값으로 반환
      ...
      if(!$pwd_valid){
        ...
        echo json_encode(array("userid" => $_SESSION['userid'], "useridx" => $login_data['user_idx'], "userlvl" => $_SESSION['userlvl']));
      }
